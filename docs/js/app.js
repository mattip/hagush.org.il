...
let interviewIds = new Set(); // candidate ids with a parsed interview (interviews/index.json)
let interviewIndex = [];       // full index.json entries (for ask.html)
...
const dataReady = Promise.all([
  ...
  // Manifest of candidates with a parsed interview — interviews/index.json,
  // regenerated by add_interview.html. Drives the grid "שואלות" badge.
  fetch("interviews/index.json", { cache: "no-cache" }).then((r) => (r.ok ? r.json() : [])).catch(() => []),
])
  .then(([people, index]) => {
    // index.json: array of objects {id, name, date, source} or legacy array of strings
    interviewIndex = index;
    interviewIds = new Set(index.map(e => typeof e === "string" ? e : e.id));
...
function createCard(person, i) {
  ...
  // Interview badge — round "שואלות" icon. Shown only when a parsed interview
  // exists for this candidate (interviews/index.json manifest). The legacy
  // candidates.json interview_day/interview_time fields no longer drive this.
  if (interviewIds.has(person.id)) {
